...
void Resistors_Calculator::on_calculate_pushButton_clicked()
{
    ...
    if(div_ratio > 0 && div_ratio < 1)
    {
        ...
        QVector<QPair<int,float>> empty(3,qMakePair(0,1)), best_dividers_indexes = get_best_dividers(dividers_list, div_ratio);
...
void Resistors_Calculator::update_resistors_tableWidget(QVector<Divider> new_dividers)
{
    for(int i = 0; i < 3; i++ )
    {
        ...
        ui->resistors_tableWidget->item(i, 3)->setText(QString::number(new_dividers[i].get_div_ratio_min()));
        ui->resistors_tableWidget->item(i, 4)->setText(QString::number(new_dividers[i].get_div_ratio_max()));
    }
}
